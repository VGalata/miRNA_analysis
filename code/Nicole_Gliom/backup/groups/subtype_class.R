...
RNA <- as.matrix(read.table('mapped_norm_RNA2.csv', sep = '\t', quote = "\"'", dec = ".", header = TRUE, row.names=1))
...
genes <- read.csv('class_genes.csv', sep='\t', header=TRUE, quote="\"'", dec=".", na.strings='')
...
## Not converted class. genes: NA entry in Gene ID column
...
# predict(lda.fit, pca.pred[to.class,]) # predict subtypes for unclassified samples
# posterior
#        1    2    3    4
# G77 0.93 0.01 0.06 0.00
# G78 0.00 0.00 0.01 0.99
# G80 0.89 0.00 0.11 0.00
# G81 0.00 0.00 0.06 0.94
# G82 0.00 1.00 0.00 0.00
# G84 0.00 0.01 0.96 0.03
# G86 0.00 0.00 0.00 1.00
# G87 0.00 0.00 0.01 0.99
# G88 0.00 0.00 0.31 0.69
# class
# 1 4 1 4 2 3 4 4 4
# save
subtypes <- c('Classical','Mesenchymal','Neural','Proneural')
classif <- data.frame(classif, LDA=classif[,'Subtype'])
classif[to.class,'LDA'] <- as.factor(sapply(predict(lda.fit,pca.pred[to.class,])$class,function(x){subtypes[x]}))
# PCA plots
...
#         G77         G78         G80         G81         G82         G84
#   Classical   Proneural   Classical   Proneural Mesenchymal      Neural
#         G86         G87         G88
#   Proneural   Proneural      Neural 
# save
classif <- data.frame(classif, SVMrad=classif[,'Subtype'])
classif[to.class,'SVMrad'] <- svm.pred
# pca plots
...
setwd("/home/valentina/Documents/HiWi_Hom/miRNA/data/Nicole")
write.table(x=classif, file="classification.csv", append=FALSE, quote=FALSE, sep = "\t", na='NA', dec='.', row.names=FALSE, col.names=TRUE)
