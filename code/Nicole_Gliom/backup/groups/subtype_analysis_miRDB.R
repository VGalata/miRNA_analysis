...
#	- Read in the interactions (from miRDB)
...
# 2013.05.16
#	- mRNA values were log. -> fold change computation changes
# 2013.06.05
#   - use stringsAsFactors=FALSE if reading in data
...
## Read in: miRNA-mRNA interactions (miRDB)
...
# 	m.genes <- interactions[which(interactions[,1]==sig.miRNA[m,'miRNA.conv']),2] # genes interacting with given miRNA
...
# 	m.genes <- interactions[which(interactions[,1]==mi.rna),2] # genes interacting with given miRNA
...
# write.table(diff.expr, file = "diffexpr_miRDB.csv", append = FALSE, quote = FALSE, sep = "\t", dec = ".", row.names = FALSE, col.names = TRUE)

setwd("/home/valentina/Documents/HiWi_Hom/miRNA/analysis_results/Groups_20130515")
diff.expr <- read.csv("diffexpr_miRDB.csv", sep = '\t', quote = "\"'", dec = ".", header = TRUE, stringsAsFactors=FALSE)
...
setwd("/home/valentina/Documents/HiWi_Hom/miRNA/analysis_results/Groups_20130605")
write.table(data.fdr, file ='diffexpr_miRDB_fdr_2.csv', append = FALSE, quote = FALSE, sep = "\t", dec = ".", row.names = FALSE, col.names = TRUE)
#
